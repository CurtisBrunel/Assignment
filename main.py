...
def inv():
    if inventory:
        ...
        for i, item1 in enumerate(inventory, start=1):
            print(f"{i}. {item1}")
    else:
...
def check_sequence(player_sequence1):
    return player_sequence1 == correct_order
...
# New line for each input
        doorchoice = input("\nEnter a choice (tree/sun/river/bird): ").lower()
...
def check_sequence(player_sequence2):
    return player_sequence2 == floor_sequence
#Loop for puzzle
...
text_print("Narrator - As you walk to the throne, you come across a familiar being.")
text_print("Hooded man - I didn't expect you to get this far.")
text_print("Hooded man - No one has ever made it this far.")
text_print("Who are you really?")
text_print("Hooded man - Me just the one who would put an end to your adventure thief.")
text_print("Narrator - The hooded man removed his cloak that was covering him.")
text_print("Narrator - His face started to deform and he became less human like. His body started to grow and his skin went black")
text_print("Hooded man - I am Anubis The God of the Afterlife")
text_print("Narrator - Although you have been startled by his transformation, you managed to draw out your weapon.")
...
boss = {
    "name": "Anubis The God of the Afterlife",
    "health": 150,
    "attack": 60,
    "defense": 10,
}
...
def boss_fight():
    text_print(f"Narrator - You go face to face against {boss['name']}.")

    #Combat loop
    while player["health"] > 0 and boss["health"] > 0:
        #Player turn
        text_print(f"Your Health: {player['health']}, {boss['name']} Health: {boss['health']}")
        text_print("Narrator - What will you do?")
        print("1. Attack")
        print("2. Defend")
        print("3. Use item")
        ...
        if action == "1":
            ...
            boss["health"] -= damage
            text_print(f"Narrator - You hit {boss['name']} for {damage} damage.")
        elif action == "2":
            #Defend
            text_print("Narrator - You braced yourself for the next attack, taking reduced damage.")
            #Temporary damage reduction
            player["defense"] += 5
        elif action == "3":
            #Use item
            if "Health Potion" in player["inventory"]:
                text_print("Narrator - You used a Health Potion and restored 25 health.")
                player["health"] += 25
                player["inventory"].remove("Health Potion")
            else:
                text_print("Narrator - You don't have any items to use.")
        else:
            text_print("Narrator - Invalid input. You lose your turn.")
        ...
        #Check Boss's health
        if boss["health"] <= 0:
            text_print(f"Narrator - You defeated {boss['name']}")
            break

        #Boss turn
        text_print(f"Narrator - {boss['name']} attacks.")
        damage = max(0, boss["attack"] - player["defense"])
        player["health"] -= damage
        text_print(f"The {boss['name']} hit you for {damage} damage!")

        #Checks player's health
        if player["health"] <= 0:
            text_print("Narrator - You have been defeated")
            break

player["weapon"] = "Sword"  # Set the player's weapon from the treasure room
boss_fight()
#Escaping the Pyramid
text_print("Narrator - The boss has fallen, you picked yourself up and make your way to the throne.")
text_print("Narrator - You took the artifact of the mummy and took his golden mask.")
text_print("Finally got it.")
text_print("Narrator - The ground starts shaking and you feel like the pyramid will collapse.")
text_print("Time to get out of here.")
text_print("Narrator - You put the items in your backpack and start to run out.")
text_print("The main entrance caved in.")
text_print("Narrator - In the distance you spot a narrow air passageway and without hesitation you start crawling out.")
text_print("Narrator - You managed to get out and run over to the time machine.")
text_print("Narrator - As you activate the time machine you see the pyramid, collapse into the ground.")
text_print("Narrator - In the distance you see a horde of Stone Creatures.")
text_print("Come on machine don't break down on me now.")
text_print("Narrator - You fiddle with the machine and right as the horde was in range you managed to get out.")

#Ending
text_print("Doctor - Agent you are back, we were worried for you since we lost connection.")
text_print("Narrator - You look around relived to the sight of a familiar location.")
text_print("Agent - It's good to be back.")
text_print("Doctor - You look rough are you alright.")
text_print("Agent - I had to deal with some minor complications, but overall I'm alright.")
text_print("Doctor - That's good. And the artifact.")
text_print("Agent - Yep and one better, I got a golden mask as well.")
text_print("Doctor - Wow well done agent.")
text_print("Doctor - Now take some time off Agent, you earned it.")
text_print("Agent - Thank you Doctor.")
text_print("Narrator - You leave the lab and head back to your apartment resting for the next mission")
